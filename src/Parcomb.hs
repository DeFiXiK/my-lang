...
parserError msg = Parser runParserError
    where
        runParserError = ParserResult (Left msg)
...
lookahead' = Parser runLookahead
    where
        runLookahead lastTok []     = ParserResult (Left "Неожиданный конец файла") lastTok []
        runLookahead lastTok (t:ts) = ParserResult (Right t) lastTok (t:ts)

lookahead2' :: Parser (Token, Token)
lookahead2' = Parser runLookahead
    where
        runLookahead lastTok (t1:t2:ts) = ParserResult (Right (t1, t2)) lastTok (t1:t2:ts)
        runLookahead lastTok ts     = ParserResult (Left "Неожиданный конец файла") lastTok ts
...
lookahead2 :: Parser (TokenType, TokenType)
lookahead2 = do
    (tok1, tok2) <- lookahead2'
    return $ (tokenType tok1, tokenType tok2)
...
consume = Parser runConsume
    where
        runConsume lastTok []     = ParserResult (Left "Неожиданный конец файла") lastTok []
...
expect et = do
    ...
    if at == et
        then skip
        else parserError $ "Ожидался токен " ++ show et
...
expectAny tts = do
    ...
    if at `elem` tts
        then consume
        else parserError $ "Ожидался один из токенов: " ++ intercalate ", " (map show tts)

tryParse :: Parser a -> Parser (Either String a)
...
cursorBehind = do
    mt <- lookbehind'
    case mt of
        Just t  -> return $ Cursor (tokenLine t) (tokenPos t)
